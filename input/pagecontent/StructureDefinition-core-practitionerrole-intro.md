...
- `identifier[misPractitionerRole].system` передается как `urn:oid:<OID медицинской организации>.<ветка МИС>.<номер МИС>.<номер экземпляра МИС>.70[.<дочерний узел>...]`; структура проверяется инвариантом
