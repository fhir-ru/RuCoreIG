...
|-------------------|---------|---------------------------|
| Необходимо передавать идентификатор случая, номер связанной стационарной карты и номер амбулаторной медицинской карты в экземпляре МИС по правилам СЭМД | identifier | Определены необязательные срезы `misEncounter`, `misInpatientRecord` и `misAmbulatoryEncounter` кратностью `0..1`; открытая нарезка распознает их по семантическому `Identifier.type`, а точная структура `system` проверяется инвариантами |
...
| Описание эпизода лечения должно соответствовать требованиям | episodeOfCare | Должен быть представлен профилем Core_EpisodeOfCare. Предлагаемая модель клинической группировки посещений приведена ниже; она вынесена на обсуждение сообщества. |
...
- Для каждого `Identifier.system` проверяются URI-форма OID и конечный узел типа; дополнительные узлы после `15`, `16` или `17` не допускаются
...
См. также [описание модели в Core_EpisodeOfCare](StructureDefinition-core-episodeofcare.html).

## Распознавание срезов идентификаторов

Динамическая часть `system` не используется для распознавания среза: принадлежность определяется стабильным семантическим кодом в `Identifier.type`. Инварианты проверяют только URI-форму OID и конечный типовой узел. Корень организации и структуру пространства МИС RuCore не ограничивает; принадлежность OID организации проверяется отдельно по реестру.

## Представление диагнозов
...
Description: "Система номера стационарной карты в МИС должна быть OID в URI-форме urn:oid:... с конечным узлом .16"
Severity: #error
