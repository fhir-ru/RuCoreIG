...
| `diagnosis.condition` | Ограничен тип | В `Core_Encounter` диагноз допускается передавать кодом и текстом или ссылкой на `Condition`. Прикладной профиль ВМП сужает это правило: диагноз указывается ссылкой на `VMP_Condition`. |
